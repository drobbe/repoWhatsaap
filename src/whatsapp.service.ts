...
import {
  saveUserNotFound,
  consultaRutUsername,
  consultaRutUserOrcob,
  saveCompromiso,
} from './helper/helper.whatsapp';
  ...
  useFactory: async (config: ConfigService) =>
    create(
      ...
      (base64Qr, asciiQR, attempt, urlCode) => {
        ...
        console.log(asciiQR);
        socketController.emit('base64Qr', {
          code: base64Qr,
          message: 'Codigo QR',
        });
      },
...
const HOOKS = [ONMESSAGE_HOOK, 'onStateChange', 'onAck', 'onAddedToGroup'];
...
export class WhatsappService implements OnApplicationShutdown {
  ...
  constructor(
    ...
    this.socket.on('whatsapp', (data: any) => {
      ...
      console.log(data);
      console.log('::::::::::::::::.. whatsapp ::::::::::::::');
      const idChatInMemory = this.activeChats.findIndex(
        c => (c.idChat = data.idChat),
      );
  ...
  async sendToSocket(chatIdInMemory: number, message: Message) {
    try {
      const branch = this.getEventOfMessage(
        message.body,
  ...
  async sendMsgUserNotFound(chatIdInMemory: number, message: Message) {
    const indexProcess = this.activeChats[chatIdInMemory].userNotFoundForm
      .indexProcess;
    this.activeChats[chatIdInMemory].userNotFoundForm.informations[
      indexProcess
    ].value = message.body;
    if (
      ...
      this.activeChats[chatIdInMemory].userNotFoundForm.informations.forEach(
        item => {
      ...
      const response = await this.sendMessageWhatsapp(
  ...
  async sendToSocketForm(chatIdInMemory: number, message: Message) {
    ...
    const positionActiveQuestion = this.activeChats[chatIdInMemory].form
      .activeQuestion;
    try {
      ...
      if (
        ...
        const branch = this.findBranchID(
          this.activeChats[chatIdInMemory].lastBranch,
          msg,
        ...
        if (branch == undefined) {
          ...
          const questions = this.activeChats[chatIdInMemory].form.questions[
            positionActiveQuestion
          ];
          let saveConsulta = [];
          if (this.config.get('USER') == '7') {
            //consulta solo para orcob
            saveConsulta = await consultaRutUserOrcob({
              idChat: this.activeChats[chatIdInMemory].idChat,
              idUser: this.activeChats[chatIdInMemory].idUsername,
              rutUser: msg,
            ...
            //consulta solo para serbanc
            saveConsulta = await consultaRutUsername({
              idChat: this.activeChats[chatIdInMemory].idChat,
              idUser: this.activeChats[chatIdInMemory].idUsername,
              rutUser: msg,
              message: questions.message,
  ...
  private async _handleChat(formatPhone, idSender): Promise<number> {
    try {
      ...
      let isRegister = await this.apiSinaptica.verifyUsername(
        formatPhone,
        this.idEmpresa,
      );
      ...
      console.log('verificando');
      if (isRegister === false) {
        isRegister = await this.apiSinaptica.createUsername(
          this.idEmpresa,
          formatPhone,
        );
      }
  ...
  private async _handleWebsocketSimpleMessage(
    idChatInMemory: number,
  ...
  optionsMenu(message: string) {
    if (message == 'Seleccione su lugar de pago:')
      return (message += this.chatBot.aditional.find(item => item.index === 0)
        .message);
    else if (message == 'Seleccione forma de pago:')
      return (message += this.chatBot.aditional.find(item => item.index === 1)
        .message);
    else if (message == 'ofertas vigentes:')
      return (message += this.chatBot.aditional.find(item => item.index === 2)
        .message);
    else if (message == 'Agenda de llamada en proceso')
      return (message += this.chatBot.aditional.find(item => item.index === 3)
        .message);
    else return message;
  }
  ...
  private async activeFormUserNotFound(
    idChatInMemory: number,
    dataWebsocket: WebSocketMessage,
  ) {
    this.activeChats[idChatInMemory].form.active = false;
    this.activeChats[idChatInMemory].userNotFound = true;
    this.activeChats[
      idChatInMemory
    ].userNotFoundForm = this.chatBot.userNotFoundForm;
    const primerEnvio = this.activeChats[idChatInMemory].userNotFoundForm
      .informations[0].message;
    await this.sendMessageWhatsapp(
      this.activeChats[idChatInMemory].idSender,
      dataWebsocket.message,
    );
    await this.sendMessageWhatsapp(
      this.activeChats[idChatInMemory].idSender,
      primerEnvio,
  ...
  getEventOfMessage(msg: string, lastBranch: number): Branch | null {
    ...
    const findedBranch = this.findBranchID(
      lastBranch,
      msg,
      this.chatBot.branchs,
    );
  ...
  async sendDefaultMesagge(idChatInMemory: number) {
    const handler =
      this.activeChats[idChatInMemory].updateUsername == true
        ? this.chatBot.default
        : 'No le entendemos. Le puedo ayudar en algo mas?';
    await this.sendMessageWhatsapp(
      this.activeChats[idChatInMemory].idSender,
      handler,
    );
  ...
  private async _activeChatWithAgent(idChatInMemory: number) {
    const response = await this.apiSinaptica.asignedAsesor(
      this.idEmpresa,
      this.activeChats[idChatInMemory].idUsername,
      this.activeChats[idChatInMemory].idChat,
    );
    ...
    return await this.sendMessageWhatsapp(
      this.activeChats[idChatInMemory].idSender,
      response.message,
    );
  }
