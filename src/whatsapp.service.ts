...
export class WhatsappService implements OnApplicationShutdown {
  ...
  constructor(
    ...
    for(const hook of HOOKS) {
      ...
      if(!url){
        this.log.log(`${hook} is disabled. Set ${env_name}`);
        continue;
      }
  ...
  async sendToSocket(chatIdInMemory: number, message: Message) {
    try {
      ...
      let branch = this.getEventOfMessage(msg, this.activeChats[chatIdInMemory].lastBranch,);
  ...
  async sendToSocketForm(chatIdInMemory: number, message: Message) {
    ...
    if(this.activeChats[chatIdInMemory].form.activeQuestion===this.activeChats[chatIdInMemory].form.questions.length - 1){
      let branch = this.findBranchID(this.activeChats[chatIdInMemory].lastBranch, msg, this.chatBot.branchs);
      ...
      //if(this.activeChats[chatIdInMemory].idSender !== '51994290430@c.us')return;
      await this.whatsapp.sendText(this.activeChats[chatIdInMemory].idSender,  this.activeChats[chatIdInMemory].form.questions[positionActiveQuestion + 1].message)
      .then(result => {
  ...
  private async isRegisterChat(number: string): Promise<any> {
    return instance.get(`verifi/telephone/${number}`)
  ...
  private _handleWebsocketSimpleMessage(data: WebSocketMessage) {
    ...
    //if(this.activeChats[idChatInMemory].idSender !== '51994290430@c.us')return;
    this.whatsapp.sendText(this.activeChats[idChatInMemory].idSender, msg)
    .then(result => {
  ...
  private async _handleAffirmation(chatIdInMemory, message: Message) {
    ...
    if(affirmation.destiny.showGoodbye) {
      //if(this.activeChats[chatIdInMemory].idSender !== '51994290430@c.us')return;
      await this.whatsapp.sendText(this.activeChats[chatIdInMemory].idSender,  this.chatBot.goodbye,)
      .then(result => {
  ...
  async greetins(idChatInMemory: number) {
    for(const message of this.chatBot.greetings) {
      //if(this.activeChats[idChatInMemory].idSender !== '51994290430@c.us')return;
      await this.whatsapp.sendText(this.activeChats[idChatInMemory].idSender, message)
      .then(result => {
  ...
  async menu(idChatInMemory: number) {
    ...
    for(const branch of this.menuBranch.branchs) {
      if(branch.enabled === true) {
        //if(this.activeChats[idChatInMemory].idSender !== '51994290430@c.us')return;
        await this.whatsapp.sendText(this.activeChats[idChatInMemory].idSender,  `*${this.optionAvailable[i]})* - ${branch.text}`,)
          .then(result => {
  ...
  sendDefaultMesagge(idChatInMemory: number) {
    //if(this.activeChats[idChatInMemory].idSender !== '51994290430@c.us') return
    this.whatsapp.sendText(this.activeChats[idChatInMemory].idSender, this.chatBot.default)
    .then(result => {
  ...
  private _activeChatWithAgent(idChatInMemory: number) {
    let clase = this;
    return instance.post(`https://realtime.sinaptica.io/v1/sinaptica/asignedAsesor`, {
      ...
      .then(function(response) {
        ...
        if(response.data.hasOwnProperty('idAsesor') &&  response.data.idAsesor > 0){
          ...
          clase._sendText(clase.activeChats[idChatInMemory].idSender, "En breves momentos un asesor se comunicara con ud. Gracias, Escriba *['Salir']* para poder Volver al chatbot");
        } else {
          clase._sendText(clase.activeChats[idChatInMemory].idSender,  response.data.message,);
        }
      })
      .catch(function(error) {
        console.log(error);
        clase._sendText(clase.activeChats[idChatInMemory].idSender, 'En estos momentos no hay asesor disponible, comuniquese mas tarde. Gracias',);
      });
  }
  ...
  async _sendText(idSender: string, text: string) {
    await this.whatsapp.sendText(idSender, text)
    .then(result => {
      return result;
    })
    .catch(erro => {
  ...
  private async downloadAndDecryptMedia(message: Message) {
    return this.whatsapp.decryptFile(message).then(async buffer => {
      ...
      if(this.mimetypes!==null &&  !this.mimetypes.some(type => message.mimetype.startsWith(type))){
        this.log.log(`${message.id} has ${message.mimetype}`);
        message.clientUrl = '';
        return message;
      }
